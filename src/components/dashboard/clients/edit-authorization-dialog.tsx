...
interface ServiceFormState {
  id: string; // Real ID if existing, temp UUID if new
  isNew: boolean;
  isDeleted: boolean;
  serviceType: string;
  billingCode: string;
...
// Convert DB service to form state
function dbServiceToFormState(service: ClientAuthorizationService & { id: string }): ServiceFormState {
  // Find the service type code from the billing code or full label
  const mapping = SERVICE_BILLING_MAP.find(m =>
    m.code === service.billing_code ||
    service.service_type?.includes(m.code)
  );
  const serviceTypeCode = mapping?.code || (service.billing_code === service.custom_billing_code ? "other" : service.billing_code || "");
  ...
  return {
    ...
    billingCode: service.billing_code || "",
    customBillingCode: service.custom_billing_code || "",
    useAuthDates: service.use_auth_dates ?? true,
    startDate: service.start_date || "",
    endDate: service.end_date || "",
    hoursPerWeek: service.hours_per_week?.toString() || "",
    hoursPerAuth: service.hours_per_auth?.toString() || "",
    unitsPerWeek: service.units_per_week?.toString() || "",
    unitsPerAuth: service.units_per_auth?.toString() || "",
    unitsUsed: service.units_used?.toString() || "0",
    useCalculatedValues: service.use_calculated_values ?? true,
    notes: service.notes || "",
  };
}
...
export function EditAuthorizationDialog({
  ...
  const [payorType, setPayorType] = useState<string>(authorization.payor_type || "");
  const [status, setStatus] = useState<string>(authorization.status || "draft");
  const [authRefNumber, setAuthRefNumber] = useState<string>(authorization.auth_reference_number || "");
  const [startDate, setStartDate] = useState<string>(authorization.start_date || "");
  const [endDate, setEndDate] = useState<string>(authorization.end_date || "");
  const [notes, setNotes] = useState<string>(authorization.notes || "");

  // Services array
  const [services, setServices] = useState<ServiceFormState[]>(() => {
    const existingServices = authorization.services?.map(dbServiceToFormState) || [];
    return existingServices.length > 0 ? existingServices : [createEmptyService()];
  });

  // Reset form when authorization changes
  useEffect(() => {
    setPayorType(authorization.payor_type || "");
    setStatus(authorization.status || "draft");
    setAuthRefNumber(authorization.auth_reference_number || "");
    setStartDate(authorization.start_date || "");
    setEndDate(authorization.end_date || "");
    setNotes(authorization.notes || "");

    const existingServices = authorization.services?.map(dbServiceToFormState) || [];
    setServices(existingServices.length > 0 ? existingServices : [createEmptyService()]);
    setError(null);
  }, [authorization]);
  ...
  // Mark service for deletion (or remove if it's new)
  const removeService = (id: string) => {
    setServices(prev => {
      const service = prev.find(s => s.id === id);
      if (!service) return prev;

      // If it's a new service, just remove it
      if (service.isNew) {
        return prev.filter(svc => svc.id !== id);
      }

      // If it's an existing service, mark it as deleted
      return prev.map(svc =>
        svc.id === id ? { ...svc, isDeleted: true } : svc
      );
    });
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Get active services (not deleted)
    const activeServices = services.filter(svc => !svc.isDeleted);

    // Validate at least one service has required fields
    const validServices = activeServices.filter(svc => svc.serviceType && svc.billingCode);
    ...
    startTransition(async () => {
      // 1. Update authorization details
      const authResult = await updateClientAuthorization(authorization.id, {
      ...
      if (!authResult.success) {
        setError(authResult.error || "Failed to update authorization");
        return;
      }

      // 2. Handle services: delete, update, add
      const deletedServices = services.filter(svc => svc.isDeleted && !svc.isNew);
      const existingServices = services.filter(svc => !svc.isDeleted && !svc.isNew);
      const newServices = services.filter(svc => !svc.isDeleted && svc.isNew);

      // Delete marked services
      for (const svc of deletedServices) {
        const result = await deleteAuthorizationService(svc.id);
        if (!result.success) {
      ...
      for (const svc of existingServices) {
        if (!svc.serviceType || !svc.billingCode) continue;

        const result = await updateAuthorizationService(svc.id, {
      ...
      for (const svc of newServices) {
        ...
        const result = await addAuthorizationService(authorization.id, {
          service_type: SERVICE_BILLING_MAP.find(m => m.code === svc.serviceType)?.fullLabel || svc.serviceType,
          billing_code: svc.billingCode === "other" ? svc.customBillingCode : svc.billingCode,
          custom_billing_code: svc.billingCode === "other" ? svc.customBillingCode : undefined,
          use_auth_dates: svc.useAuthDates,
  ...
  // Filter out deleted services for display
  const visibleServices = services.filter(svc => !svc.isDeleted);
    ...
    <Dialog open={open} onOpenChange={handleOpenChange}>
      ...
      <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Update authorization details and services.
          </DialogDescription>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              ...
              <Button type="button" variant="outline" size="sm" onClick={addNewService}>
