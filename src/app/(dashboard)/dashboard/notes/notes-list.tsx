...
export function NotesList({ initialNotes, clients }: NotesListProps) {
  ...
  const filtered = notes.filter((note) => {
    ...
    if (clientFilter !== "all" && clientFilter !== "unassigned" && note.clientId !== clientFilter) return false;
    if (searchQuery) {
      const q = searchQuery.toLowerCase();
      if (!note.body.toLowerCase().includes(q) && !(note.clientName || "").toLowerCase().includes(q)) return false;
    }
    return true;
  });
    ...
    <div className="space-y-4">
            ...
            <Command>
              ...
              <CommandList>
                ...
                <CommandGroup>
                  <CommandItem
                    value="all"
                    onSelect={() => { setClientFilter("all"); setClientFilterOpen(false); }}
                  >
                  ...
                  <CommandItem
                    value="unassigned"
                    onSelect={() => { setClientFilter("unassigned"); setClientFilterOpen(false); }}
                  >
                    <Check className={cn("mr-2 h-4 w-4", clientFilter === "unassigned" ? "opacity-100" : "opacity-0")} />
                    Unassigned
                  </CommandItem>
                  {clients.map((client) => (
                    <CommandItem
                      key={client.id}
                      value={client.name}
                      onSelect={() => { setClientFilter(client.id); setClientFilterOpen(false); }}
                    >
      ...
      {filtered.length === 0 ? (
        ...
        <div className="space-y-2">
          {filtered.map((note) => (
            <div key={note.id} className="group rounded-lg border bg-card p-4 transition-colors hover:bg-muted/30">
              <div className="flex items-start justify-between gap-3">
                  ...
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    {note.clientId && note.clientName && (
                      <Link href={`/dashboard/clients/${note.clientId}`}>
                        <DashboardStatusBadge tone="default" className="w-fit cursor-pointer gap-1 hover:bg-muted">
                          <User className="h-3 w-3" />
                          <span className="max-w-[150px] truncate">{note.clientName}</span>
                        </DashboardStatusBadge>
                      </Link>
                    )}
                ...
                <DropdownMenu>
                  ...
                  <DropdownMenuContent align="end">
                    ...
                    <DropdownMenuSeparator />
                    <DropdownMenuItem className="text-destructive" onClick={() => setDeleteTarget(note)}>
      ...
      <NoteFormDialog
        open={dialogOpen}
        onOpenChange={(open) => { setDialogOpen(open); if (!open) setEditingNote(undefined); }}
        note={editingNote}
        clients={clients}
